...
class ScrollContainer(ScrollView):
    def __init__(self, **kwargs):
        ...
        # print('joko')
        zz = 'testst'
        Logger.info('ScrollContainer: {}'.format(zz))
...
class ScrollGrid(GridLayout):
    ...
    def __init__(self, **kwargs):
        ...
        Logger.info('ScrollGrid: {}'.format(len(MainView.dic_store.keys())))
        ...
        for key in scroll_grid_dic:
            ooo +=1
            Logger.info('ScrollGrid: add exp label count {}'.format(ooo))
...
class Mo(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        if len(self.gif_wrap.children) < 1:
            ...
            xxx.volume=0
            # xxx.anim_delay = 0
            # print(xxx.anim_delay)
            self.gif_wrap.add_widget(xxx)
    def re(self):
        # print('remove widget')
        Logger.info('Mo: clear tip Modal {}'.format(self.ids))

        # self.children[0].anim_delay = -1
        # print(self.children[0].anim_delay)
        # print(self.ids)
...
class StyleLabel(BoxLayout):
    ...
    def exp_converter(self,string_to_parse):
        ...
        if parsed_cur == 'RSD':
            ...
            self.converted_exp_str_one = '{} {} = {} RSD'.format(parse_val, parsed_cur, round(to_rsd,2))
            self.converted_exp_str_two = '{} {} = {} DOL'.format(parse_val, parsed_cur, round(to_dol,2))


    def open_modal(self,tex):
        Logger.info('StyleLabel: children {}'.format(self.children))
        exp_m = ExpModal()
        exp_m.open(self)
        colo = kivy.utils.get_color_from_hex('#31ddd0')
        self.exp_converter(tex)
        # print(self.exp_converter(tex))
...
# Show
    def inits_anim(self,im):
        ...
        animation = Animation(x=330, y=400, duration=4, t='out_bounce')+Animation(x=330, y=20, duration=4, t='out_elastic')
...
# Callback Show_Edits
    def store_expense_with_currency(self,ex_name,ex_value,ex_currency):
        Logger.info('MainView: Storing expense currnency: name {} value {} curr {}'.format(ex_name,ex_value,ex_currency))
